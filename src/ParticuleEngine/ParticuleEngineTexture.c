...
PC_Texture* PC_LoadTexture(const char* path)
{
    ...
    return pc_texture;
    #elif defined(NDS_MODE)
    PC_Texture* texture = NULL;
    for (List_node*cur=NULL; ForEach(AllTextures,&cur);)
    {
        PC_Texture* tex = (PC_Texture*)cur->data;
        if (strcmp(tex->path, path) == 0)
        {
            texture = tex;
            break;
        }
